        ...
        autoplay={{delay:3000}}>
            {listings.map(({data, id}) => (
                <SwiperSlide key={id} onClick={()=>navigate(`/category/${data.type}/${id}`)}>
                    ...
                    <p className='text-[#f1faee] absolute left-1 bottom-3 font-medium max-w-[90%] bg-[#e63946] shadow-lg opacity-90 p-2 rounded-tr-3xl'>${(data.discountedPrice ?? data.regularPrice).toString()
                            .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
